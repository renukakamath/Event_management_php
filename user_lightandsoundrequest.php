...
<center>
	...
	<form>
     ...
     $q="SELECT * FROM `lightsound_request` INNER JOIN `lightsound`  USING (`lightsound_id`) INNER JOIN `customer`  USING (`customer_id`)  inner join lightandsound using (light_sound_id) where customer_id='$uid'";
     $res=select($q);
     $sino=1;
    ...
    foreach ($res as $row) {?>
    	<tr>
    		...
    		<?php 
    			...
    			if ($row['is_status']=="accept") {
    				?>


<td><a class="btn btn-success" href="user_lightmakepayment.php?bid=<?php echo $row['srequest_id'] ?>&amount=<?php echo $row['amount'] ?>">Payment</a></td>
